# Low-Cost Mechanical Powder Dispenser Design Review

- **Trajectory ID:** `faeecd89-ea28-403b-94f3-c8402a79e78a`
...
```
Please review the attached design package for a new low-cost mechanical powder dispenser ("powder-excavator") intended for self-driving laboratories handling dozens-of-microns inorganic powders (catalysts, ceramics, salts; some cohesive / hygroscopic / triboelectrically charged).

Mechanism in one paragraph: an elongated half-cylinder trough is suspended between TWO parallel vertical arms by a single horizontal pivot pin that passes through clearance holes in both arms and through pivot bosses on each side wall of the trough. The arms are bolted to a gantry carriage and ALWAYS stay vertical. To pick up powder the gantry dips the trough straight down into a powder bed. To deposit, the gantry translates the trough in X into a fixed, wall-/post-mounted sawtooth ledge positioned at trough-lip height; a hooked lip on the trough's upper long edge catches a sawtooth tooth, and continued X-travel applies a torque about the pin that rotates ONLY the trough, pouring powder out at a controlled X-coordinate. There are NO actuators on the bucket. The intended manufacturing target is fully 3D-printed (PETG / nylon for the prototype, with a stock dowel pin / shoulder bolt as the pivot).

Attached files:

  panel-A-orthographic.png   Front / side / top orthographic views with D
                             and L = 3D dimensions, two arms, pivot pin
                             (red), pivot bosses, hooked lips.
  panel-B-pivot-detail.png   Detail of the pivot region showing the pin
                             through both arms and both bosses, retaining
                             clip, hooked lips, rotation indicator.
  panel-C-isometric.png      3D / isometric view of the assembly hung from
                             a gantry beam + carriage.
  panel-D-mechanism.png      Three-step mechanism of action:
                             (1) dip down, (2) lift & X-transport,
                             (3) tip against the wall-mounted sawtooth
                             ledge — arms remain vertical throughout.
  original-sketch.jpg        Hand-drawn concept sketch the diagrams were
                             recreated from (for ground-truth reference).
  README.md                  Full design brainstorm.
  brainstorming-and-literature.md
                             Literature-aware prior-art writeup with
                             references and a section distilling the
                             accelerated-discovery.org community thread.

I would like high-effort feedback on:

1. Mechanical soundness. Are there geometric pitfalls I'm missing? In    particular: pin location relative to centre of mass (loaded vs.    empty), tendency of the trough to swing during X-transport, whether    the hooked-lip / sawtooth engagement is actually self-aligning,    and the worst-case force the gantry must apply in X to drive the    tilt.

2. Powder-handling realism for the target particle size and powder    classes. Is this geometry plausible for cohesive / hygroscopic /    triboelectric powders, or does it predictably fail (bridging,    sticking, static cling, residue retention)? What surface treatments    or ESD-mitigation would you recommend for a 3D-printed prototype?

3. Repeatability / dose CV. What is a realistic dose CV for a scoop    like this (with and without a strike-off bar) on the relevant    powder classes, and what would be the cheapest way to measure it?

4. Suggested benchmark experiments and instrumentation to characterise    this device against published precision dispensers (Quantos,    Chemspeed, etc.) — recognising that the excavator is positioned as    a *bulk-transfer* tool upstream of a precision dispenser, not as a    competitor.

5. Specific suggestions to improve the four diagrams (clarity, missing    labels, missing sections, suggested additional subpanels).

6. Anything in the README / brainstorming-and-literature.md that you    would push back on, expand, or cite differently.
